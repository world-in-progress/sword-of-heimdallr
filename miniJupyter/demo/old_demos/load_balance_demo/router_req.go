...
func worker_task() {
	...
	for {
		...
		worker.SendMessage(identity, empty, "OK", 0)
	}
}
...
func main() {
	...
	for {
		// Poll with available workers check
		...
		if available_workers > 0 {
			...
			sockets, err = backendPoller.Poll(-1)
		}
		if err != nil {
			break //  Interrupted
		}
		...
		if sockets[0].Events&zmq.POLLIN != 0 {
			...
			if string(client_id) != "READY" {
				empty := parts[3]
				reply := parts[4]
				frontend.SendMessage(client_id, empty, reply, 0)
				client_nbr--
				if client_nbr == 0 {
		...
		if sockets[1].Events&zmq.POLLIN != 0 {
			...
			empty := parts[1]
			request := parts[2]
			backend.SendMessage(worker_queue[0], empty, client_id, empty, request, 0)
